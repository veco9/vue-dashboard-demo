...
export function useGridDragDrop(
  ...
  function handleDrag(e: DragEvent, itemOverrides: Partial<LayoutItem> = {}): void {
    ...
    try {
      (item as any).wrapper.style.display = "none";
    } catch {
      // Ignore errors
    }

    Object.assign((item as any).state, {
      top: pointerY.value - bounds.top,
      left: pointerX.value - bounds.left,
    });

    const gridPos = (item as any).calcXY(pointerY.value - bounds.top, pointerX.value - bounds.left);
  ...
  function handleDragStart(e: DragEvent, itemOverrides: Partial<LayoutItem> = {}): void {
    ...
    const colWidth = computeColumnWidth((gridRef.value as any)?.state?.width ?? 800, gridGap[0]);
  ...
  function handleDragEnd(): LayoutItem | undefined {
    ...
    try {
      (item as any).wrapper.style.display = "";
    } catch {
      // Ignore errors
    }
  ...
  function handleItemMove(i: string | number, newX: number, newY: number): void {
