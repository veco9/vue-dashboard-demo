...
import type { AgChartProps } from "@/components/widgets/AgChart.vue";
import type { WidgetHeaderProps } from "@/components/widgets/WidgetHeader.vue";
    ...
    try {
      ...
      const callbacks = {
        updateData: (newData: Record<string, unknown>) => {
          ...
          if (current) {
            updateRuntime(widgetId, { data: { ...current, ...newData } as any });
          }
        },
      };
      ...
      console.warn("initializeWidget error!", { widget, error });
      updateRuntime(widgetId, {
        loading: false,
        header: undefined,
        data: undefined,
