        ...
        <template #item="{ data: widget }">
          ...
          >
            <div class="dashboard-palette-item-inner">
              ...
              >
                <i :class="getWidgetIcon(widget.type)" class="dashboard-palette-item-icon"></i>
              </div>
...
watch(
  () => props.breakpoint,
  () => nextTick(() => (carouselRef.value as any)?.createStyle?.()),
);
...
function getIconColor(type: WidgetType): string {
...
function onItemRemoved(item: DashboardWidget, index: number) {
  if (palettePage.value > 0 && index % numVisible.value === 0) {
    (carouselRef.value as any)?.step?.(1, palettePage.value - 1);
  }
}
